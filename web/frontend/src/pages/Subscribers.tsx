  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-100">Subscribers</h1>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select
          className="rounded-md bg-slate-800 border border-slate-700 px-3 py-1.5 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-brand-500"
          value={statusFilter}
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-900">
            {table.getHeaderGroups().map(hg => (
              <tr key={hg.id}>
                {hg.headers.map(h => (
                  <th key={h.id} className="px-3 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wide">
          ...
          <tbody className="divide-y divide-slate-800">
            {isLoading ? (
              <tr><td colSpan={columns.length} className="px-3 py-6 text-center text-slate-500">Loading…</td></tr>
            ) : table.getRowModel().rows.length === 0 ? (
              <tr><td colSpan={columns.length} className="px-3 py-6 text-center text-slate-500">No subscribers found.</td></tr>
            ) : (
              table.getRowModel().rows.map(row => (
                ...
                >
                  {row.getVisibleCells().map(cell => (
                    <td key={cell.id} className="px-3 py-2.5 text-slate-300">
...
function AddForm({ onSubmit, loading }: AddFormProps) {
  ...
  return (
    <form onSubmit={submit} className="rounded-xl border border-slate-700 bg-slate-900 p-4 grid grid-cols-2 gap-3 md:grid-cols-3">
      <div>
      ...
      <div>
        <label className="text-xs text-slate-400 mb-1 block">Package ID *</label>
        <Input required value={form.package_id} onChange={set('package_id')} placeholder="e.g. P1" />
      </div>
      <div>
      ...
      <div>
        <label className="text-xs text-slate-400 mb-1 block">Custom Price ($)</label>
        <Input type="number" step="0.01" value={form.custom_price} onChange={set('custom_price')} placeholder="Leave blank for default" />
      </div>
