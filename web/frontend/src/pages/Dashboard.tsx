...
export default function Dashboard() {
  ...
  if (error || !data) return <p className="text-red-400">Failed to load dashboard.</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <StatCard label="Active" value={data.active_subscribers} icon={UserCheck} iconColor="text-emerald-400" />
        <StatCard label="Inactive" value={data.inactive_subscribers} icon={UserX} iconColor="text-slate-500" />
        <StatCard label="Due Today" value={data.due_today} icon={CalendarClock} iconColor="text-yellow-400" />
      </div>
          ...
          <div className="space-y-3">
            {notifStatus && Object.entries(notifStatus as Record<string, { enabled: boolean }>).map(([svc, cfg]) => (
              <div key={svc} className="flex items-center justify-between text-sm">
                ...
                <span className={cfg.enabled ? 'text-emerald-400' : 'text-slate-600'}>
